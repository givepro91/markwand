...
describe('formatProjectWikiHandoffBrief', () => {
  it('formats an AI-ready markdown handoff with evidence and risk context', () => {
    ...
    expect(text).toContain('# AI Handoff: markwand')
    expect(text).toContain('> Paste this into Claude, Codex, Cursor, or another AI tool')
    expect(text).toContain('## What I Need From You')
    expect(text).toContain('- Primary task: Repair risky document references (high).')
    expect(text).toContain('## Current Situation')
    expect(text).toContain('- Markwand reading: Repair broken or stale document references before relying on summaries.')
    ...
    expect(text).toContain('## Additional AI Task Suggestions')
