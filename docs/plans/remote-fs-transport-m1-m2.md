...
| `src/main/ipc/workspace.ts:196` (project:scan-docs) | `scanDocs(...)` generator | **M1.x defer** (Evaluator CONDITIONAL PASS 2026-04-21) — `services/scanner.ts`의 Doc chunk 스트리밍 로직을 `LocalScannerDriver`에 이식하는 작업이 단순 위임이 아닌 비자명한 refactor임(frontmatter 파싱 + 청크 분할 + 경로 유지). 순환 import 없이 이식하려면 `LocalScannerDriver.scanDocsAsDocs(...)` 같은 app-layer 헬퍼를 새로 만들거나, Doc composition을 IPC 핸들러로 끌어올려야 한다. M4 watcher 작업과 묶어 처리 예정. 현재 handler는 `services/scanner.scanDocs`를 직접 호출하고, 그 내부의 `fs.promises.stat`은 `localTransport.fs`를 경유하지 않음 — M1 완결성 갭. | M |
...
| RM-7 | 완결성 갭 | `project:scan-docs` 미위임 (Evaluator Major 2026-04-21) — services/scanner.scanDocs 의 내부 fs.promises.stat 이 transport 우회 | **Medium** | M1.x 후속 PR에서 `LocalScannerDriver`에 Doc chunk streaming 이식 또는 IPC 핸들러에 Doc composition 끌어올림. M4 watcher 작업과 묶어 진행. |
