...
- **Blocker**: none
- **Last Activity**: Drift occurrence ignore + tool tabs — 무시 기준을 경로가 아닌 개별 참조 occurrence key(line/col/raw/path)로 변경하고, 우측 문서 도구를 문제/목차 탭 + 닫기 가능한 레일로 재구성. QA: targeted vitest 4 PASS · typecheck PASS · full test 418 PASS · build PASS (2026-05-02).
...
## Recently Done
1. Drift occurrence ignore + tool tabs — 같은 경로 반복 참조도 개별 무시, 문제/목차는 탭 전환 + 레일 닫기 가능.
2. Drift review rail UX — 우측 상시 패널에서 참조 이동/무시/복사/재검증을 문서 읽기 중 유지.
3. Drift table-line jump hardening — 표 안 참조 라인 이동 + Drift 패널 검색 오염 차단.
