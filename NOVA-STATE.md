...
- **Blocker**: none
- **Last Activity**: SSH-aware search backend — preload/main `workspace:search-docs`, bounded body cache/read budget(local 80, SSH 8), CommandPalette project scope 연결. QA: typecheck PASS · vitest 393 PASS · build PASS (2026-05-01). lint는 로컬 eslint 바이너리 부재로 실행 불가.
...
## Recently Done
1. SSH-aware ⌘K search backend — 검색 IPC/본문 캐시/원격 읽기 예산을 추가해 검색 기능을 실제 동작 경로로 연결.
2. Project Wiki UI containment — 긴 파일명/점수 배지가 하단 3컬럼 밖으로 새지 않도록 shrink/ellipsis 규칙과 회귀 테스트 추가.
3. Decision Timeline — Project Wiki가 plan/design/review/release 문서를 분류해 최신 의사결정 흐름을 타임라인으로 보여줌.
