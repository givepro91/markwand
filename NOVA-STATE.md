...
## Current
- **Goal**: v0.4.0-beta.8 macOS 무료 설치 안내 정정 공개 완료.
- **Phase**: **released** — beta.8 packaging/docs follow-up commit/tag/push/GitHub prerelease 완료. 다음은 Apple Developer ID 공증 여부 결정 또는 ESLint 게이트 정비.
- **Blocker**: none
- **Recently Done (3)**:
  1. v0.4.0-beta.8 — beta.7 helper rollback. `.command` 헬퍼 제거(동일 Gatekeeper 차단 확인), 무료 배포 fallback을 검증된 Terminal 한 줄로 정정. QA: typecheck PASS · vitest 374/374 · dist:mac:free PASS · quarantined install fallback smoke PASS.
  2. v0.4.0-beta.6 — Project Wiki MVP(Start Here/Risk Board/Knowledge Map/AI task prompt), 동일 basename 상대경로 표시, 새 UI/아이콘, 무료 ZIP 배포, 종료 cleanup watchdog.
  3. v0.4.0-beta.5 — 좌측 트리 실시간 동기화 (incremental 'add' fs:change + activeProjectId prefs persist). 사용자 dogfood 회귀 2건 root cause 두 layer fix.
- **Last Activity**: v0.4.0-beta.8 GitHub Release 공개 — prerelease ZIP arm64/x64 업로드, beta.7 deprecated note 추가, sha256 기록 (2026-05-01).
- **Remote**: git@github-givepro91:givepro91/markwand.git (main) — origin = `12bc1b3` (v0.4.0-beta.8, state update pending). tag `v0.4.0-beta.8` 원격 반영 완료. GitHub Release prerelease: https://github.com/givepro91/markwand/releases/tag/v0.4.0-beta.8 (ZIP arm64 `28daa2ad…89fde` · x64 `6c9faeea…31d74`).
...
| **useAnnotations capture true 글로벌 listener** | `document.addEventListener('mousedown'/'keyup', ..., true)` — 다른 모달/CommandPalette 와 capture phase 충돌 잠재. mouseup 에서 isDragging false 복원으로 1턴 회복하나, `containerRef` 범위로 좁히면 정리됨 (Review 2026-04-26 Info #1) | v0.5 Low |
| **CSS.highlights find-match + annotation 색상 모호** | 둘 다 노랑계열 (`#ffd54f` vs `rgba(255,213,79,0.8)`) — 사용자가 ⌘F 검색 + 하이라이트 동시 사용 시 구분 어려움. 라이프사이클 race 는 없음 (key 분리) (Review 2026-04-26 Info #2) | v0.5 Low — 다색 팔레트 시 분리 |
| **sidecar / fs:read-doc / drift symlink resolution 미수행** | `assertInWorkspace` 가 `realpath` 미사용 — workspace 안 심볼릭 링크 시 link target 의 부모 디렉토리에 sidecar 생성 가능. 기존 fs/drift 와 동일 미처리, S7 신규 취약점 X (Review 2026-04-26 Info #3) | v0.5 Medium — workspace 전체 realpath 가드 |
...
## Last Activity
- **v0.4.0-beta.5 release commit → PASS (2026-04-30)** — beta.4 dogfood 보고 "신규 파일/폴더 좌측 트리 미반영, 프로젝트 빠져나갔다 들어가야 보임" 두 layer 에서 끝까지 fix. **A. watcher → renderer incremental 'add' 경로**: setProjectIdResolver/setDocsCacheInvalidator setter 패턴(workspace ↔ watcher circular import 회피), fs:change 페이로드에 mtime/name/projectId 추가, project:scan-docs 에 force 옵션, watcher 이벤트 시 main docsCache 자동 무효화, findProjectIdByPath{In} pure helper. useDocs refreshKey>0 force=true + 'add' 분기 incremental appendDocs (mtime/name/projectId 가드). **B. activeProjectId persist (root cause)**: 미persist 시 dev hot-reload / 재시작에서 viewMode='project' but activeProjectId=null 모순으로 ProjectView mount 안 됨 → fs:change listener 없음 → 신규 파일 안 보임. StoreSchema + electron-store schema/defaults 에 activeProjectId 추가(미정의로 부팅 시 strip 되던 회귀 차단). validators ALLOWED_PREFS_KEYS 'activeProjectId' 화이트리스트. App.tsx prefs.get('activeProjectId') 부팅 복원 + projectPersistArmedRef 가드(default null 이 inject 값 덮어쓰는 race 차단). **C. Self-QA First — 사용자 dogfood 떠넘기지 않고 책임 끝**: 처음에 자동 시뮬 7건만 통과 후 사용자에게 떠넘긴 패턴이 사용자 짜증 유발. 사용자 dev 직접 종료/재시작 + 디버그 trace 캡처 + 실측 root cause 발견(electron-store schema strip · boot race). 자동 통합 시뮬 7건 + useDocs 8건 + workspace findProjectIdByPathIn 1건 + validators allowlist 4건 = 신규 20건, 회귀 0. typecheck PASS · vitest 339/349(회귀 0, 10 fail pre-existing drift/scanner). package.json 0.4.0-beta.4 → 0.4.0-beta.5. 변경 13 파일(.nova-orchestration.json + 12 src/test). 다음: tag/push/dist:mac/SHA256/GitHub Release 사용자 명시 승인 후. **회고 메모**: 자동 시뮬과 실 환경 갭에 store 영속 layer + boot order race 까지 포함해야 사용자 dogfood 떠넘기지 않음. | 2026-04-30T
- **v0.4.0-beta.4 GitHub Release 공개 → PASS (2026-04-29)** — 이전 세션 메모대로 `gh auth switch --user givepro91` + `gh api repos/givepro91/markwand/releases POST` 직접 호출 → release id `315207497` 생성. `gh release upload v0.4.0-beta.4` 로 양쪽 DMG asset 업로드 성공 (digest 일치 검증). 공개 URL: https://github.com/givepro91/markwand/releases/tag/v0.4.0-beta.4 (prerelease=true · target=main · author=givepro91 · DMG arm64 `a657927a…955e` · x64 `b5d4bdec…5bd4`). 이번 세션 commit 2건: `481c740` release skel · `604520e` SHA256. tag `v0.4.0-beta.4` 원격 반영 완료. 세션 클로징 — beta.4 dogfood 피드백 수신 대기. | 2026-04-29T
- **v0.4.0-beta.4 DMG 빌드 + SHA256 → PASS (2026-04-29)** — `pnpm dist:mac` 1회 시도로 양쪽 빌드 성공. **arm64** `a657927ac82bf2e41fc7d32dcb315750f9d5545dbca9cf920413270ca909955e` (136MB) · **x64** `b5d4bdec2c91deb2314c37e12ae5306958fe4572d6075e66db957bd851025bd4` (143MB). `docs/install-macos.{md,en.md}` + `docs/release-notes/v0.4.0-beta.4.md` SHA256 placeholder 실제 해시로 교체. 다음: tag `v0.4.0-beta.4` push + GitHub Release prerelease 공개. | 2026-04-29T
- **v0.4.0-beta.4 Release 준비 → PASS (2026-04-29)** — beta.3 dogfood 피드백 6건을 묶은 새로고침 UX 보강 미니 릴리스. 사이클 6회 self-QA: (A) 글로벌 새로고침 + ⌘R + 버전 표시 / (B) 풀스크린 오버레이 깜빡임 제거 / (C) 새로고침 시 docs+content 재로드 / (D) 이미지 cache busting (로컬 + SSH) / (E) FileTree 스크롤 보존 (appendDocs path-dedup) / (F) RecentDocsPanel "+N개 더" 인박스 라우팅. **변경 30 파일**: 9 신규 (lib/loadingOverlay+test, lib/imageSrc+test, hooks/useReloadOnRefresh+test, hooks/useGlobalHotkey.test, components/Sidebar.test, components/RecentDocsPanel.test, global.d.ts) + docs/release-notes/v0.4.0-beta.4.md + 21 수정. **자가 검증 단위 테스트 31건 PASS**: Sidebar 5 / useGlobalHotkey 5 / loadingOverlay 7 / store +7 / useReloadOnRefresh 5 / imageSrc 4 / RecentDocsPanel 3. 사용자 dogfood 회귀 3건(FileTree 스크롤 리셋·이미지 stale·RecentDocsPanel +N 접근 불가) 모두 회귀 차단 단위 테스트 동반. typecheck PASS · vitest **319/329** (회귀 0, 10 pre-existing drift/scanner). package.json 0.4.0-beta.3 → 0.4.0-beta.4, `docs/install-macos.{md,en.md}` 버전 placeholder + `__SHA256_*__` (빌드 후 별도 commit). 다음: pnpm dist:mac → SHA256 갱신 commit → tag/push/GitHub Release prerelease 공개 (사용자 명시 승인 후). | 2026-04-29T
- **v0.4.0-beta.3 GitHub Release 공개 → PASS (2026-04-28)** — `gh release create` 가 jay-swk active 계정에서도 "workflow scope may be required" 오작동(repo scope 만으로 충분한 API 요건). **우회**: `gh auth switch --user givepro91` (givepro91 이 owner) → `gh api repos/givepro91/markwand/releases POST` 직접 호출 → release id 314552564 생성. 그 후 `gh release upload v0.4.0-beta.3` 로 양쪽 DMG asset 업로드 성공. 공개 URL: https://github.com/givepro91/markwand/releases/tag/v0.4.0-beta.3 (prerelease=true · target=main · author=givepro91). **이번 세션 commit 5건**: cb871b6 S7 feat · 22ffb49 review state · 67767cc release skel · 09ee687 SHA256 · (이 항목) 세션 클로징. 릴리스 절차 v0.5 메모: workflow scope 미보유 계정 + chpwd 자동 switch 환경에서는 `gh api ... releases POST` + `gh release upload` 2단계 우회가 안정. | 2026-04-28T
- **v0.4.0-beta.3 DMG 빌드 + SHA256 → PASS (2026-04-28)** — `pnpm dist:mac` 1회 시도로 양쪽 빌드 성공(EAGAIN 없음). **arm64** `b73cbb3d1d6d69c2c1eab3d2d8d31639867eb016c96f8d5b1f157b3f6a4e7d53` (136MB) · **x64** `ca85b1b8d89131939693f09820c6ec88bbf699d92bd97c69e7baa0fc057e0cb0` (143MB). `docs/install-macos.{md,en.md}` + `docs/release-notes/v0.4.0-beta.3.md` SHA256 placeholder 실제 해시로 교체. 다음: tag `v0.4.0-beta.3` push + GitHub Release prerelease 공개. | 2026-04-28T
- **v0.4.0-beta.3 Release 준비 → PASS (2026-04-28)** — package.json 0.4.0-beta.2 → 0.4.0-beta.3, `docs/release-notes/v0.4.0-beta.3.md` 신규(S7 Annotation MVP 사용법 + 구현 핵심 + UX 가드 + 검증 + 한계 + 다음 v0.5 후속), `docs/install-macos.{md,en.md}` 버전 placeholder 갱신 (SHA256 은 빌드 후 보강 commit). typecheck PASS · vitest **283/293**(회귀 0, 10 fail pre-existing). 사후 자가 점검 — anchor.test.ts 15 + useAnnotations.test.tsx 6(자가 검증 안전망) 모두 PASS. test 파일에서 unused `useRef` import 제거 (typecheck noUnusedLocals 회복). 다음: pnpm dist:mac → SHA256 갱신 commit → tag/push/GitHub Release prerelease 공개. | 2026-04-28T
- **v0.4 S7 Annotation MVP → PASS (2026-04-26)** — Plan §S7 그대로 구현: dom-anchor-text-quote + CSS Custom Highlight API + sidecar JSON, 노랑 1색, 로컬 .md only. **신규 9 파일**: `lib/annotation/{types,anchor,sidecar,dom-anchor.d}.ts` · `state/annotationStore.ts` · `hooks/useAnnotations.ts` · `components/AnnotationToolbar.tsx` · `main/ipc/annotation.ts` · `lib/annotation/anchor.test.ts`(15) · `hooks/useAnnotations.test.tsx`(6). **수정 8 파일**: MarkdownViewer 통합(ref + hook + orphan 뱃지 sticky bar) · preload index/types · main index registerAnnotationHandlers · validators(zod schema + parse) · humanizeError 2키 · globals.css `::highlight(markwand-annotation-highlight)` 노랑 0.8 + #1f2328 · ko/en annotation 블록(highlight/remove/toolbarLabel/disabledTooltip/saveFailed/tooLong/orphanBadge). package.json: `dom-anchor-text-{quote,position}` + pnpm overrides 로 `dom-anchor-text-position@^5.0.0` 강제(jsdom 25 Range API 비호환 회피). **S7.1 Evaluator(nova:senior-dev) CONDITIONAL** → M-1 toRange hint 시그니처(options.hint:number) · M-2 readFile length 재가드 · m-1 EMPTY_ANNOTATIONS 상수 즉시 반영. **S7.2 Evaluator(nova:qa-engineer) CONDITIONAL** → C-1 라이트 시인성(0.5→0.8 + 검정 글자) · M-1 saveFailed→toast.error · M-2 orphan 뱃지 sticky · M-3 키보드 autoFocus + ESC dismiss · M-4 rAF throttle · m-2 effect cleanup · m-3 tooLong toast.info 전부 반영. **사용자 dogfood 회귀 3건 자가 분석 후 fix**: (a) 드래그 도중 toolbar 출현 — `selectionchange` 가 native drag 진행 중 매 픽셀 발화하던 race. **isDraggingRef 도입** — mousedown 시 true + toolbar hide, selectionchange 는 drag 중 컴퓨트 skip, mouseup 시점에 한 번만 결정 (keyup Shift/Cmd/Ctrl/A 도 지원). (b) 제거 버튼 무동작 — toolbar ✕ click 이 root.click 으로 bubble → caretRangeFromPoint(toolbar 위치) hit 없음 → setToolbar(HIDDEN) → button onClick 시점 hitId=null. **fix**: `[data-annotation-toolbar]` closest 검사 + button stopPropagation + toolbar div onClick stopPropagation. (c) doc 전환 시 동일 단어 잠깐 highlight — `ProjectView.loadDoc`가 `setSelectedDoc(즉시) → await readDoc → setDocContent` 순서라 basePath × stale content race. **fix**: `prevDocPathRef` 로 docPath 변경 first cycle 매칭 skip + 즉시 registry.delete. **자가 검증 단위 테스트 6/6 PASS** — RTL renderHook + jsdom 시뮬, drag/mousedown/mouseup 라이프사이클 전부 자동 회귀 차단 (drag 진행 중 미표시 · mouseup valid → visible · collapsed → hidden · 공백만 → hidden · toolbar 자체 mousedown 격리 · docPath race). 추가 baseline fix: `useDocs.ts:10` `Object.freeze([]) as Doc[]` → `as unknown as Doc[]` (베타.2 시점 잠재 typecheck 회귀). **CLAUDE.md 신규**: 자가 검증 우선 정책 명문화(코드 분석 → 자동 시뮬 → dogfood 순). typecheck PASS · vitest **283/293**(베타.2 baseline 277 + 새 6, 회귀 0, 10 fail 모두 pre-existing drift/scanner). **릴리스 미수행**: package.json version 0.4.0-beta.2 그대로, 태그/DMG/GitHub Release 모두 사용자 승인 후로 보류. | 2026-04-26T
- **v0.4.0-beta.2 Critical Hotfix → CONDITIONAL PASS (2026-04-25)** — beta.1 의 4종 회귀를 한 번에 정리. **(1)** 로컬 스캔 4500배 회귀: `9380f4a` 의 chokidar 활성화가 `~/develop` 큰 트리에서 libuv 4-스레드풀 점거 → swk(15279 폴더) 첫 `scanProjects` 가 131,486ms. **fix**: `src/main/index.ts` `startWatcher` 5s defer + `src/main/services/watcher.ts` `depth: 10 → 4`. 실측 131,486ms → **148ms (888배)**. **(2)** Maximum update depth(FilterBar): `src/renderer/hooks/useDocs.ts` `useFrontmatterIndex`/`useDocsOf` selector 가 매 호출 새 객체/배열 반환 → Zustand 강제 re-render. **fix**: Set 참조 구독 + `useMemo` + 모듈 상수 `EMPTY_DOCS = Object.freeze([])`. **(3)** SSH drift ENOENT 도배: `src/main/ipc/drift.ts` `localTransport` 하드코딩 4곳 → `resolveTransportForPath` 경유 `transport.fs.*`. `assertInWorkspace({posix: isSsh})`. **(4)** dev DevTools 노이즈: `scripts/dev.mjs` 신규 stderr 필터 wrapper(Autofill/VE language-mismatch/devtools://devtools/ 패턴) + `package.json` `dev:raw` 비상구. **부수**: SSH 30s 타임아웃 + 재시도/제거 CTA + 오버레이 main 한정. i18n loading.timeoutTitle/errorTitle/retry/removeWorkspace/switchHint ko/en. **Evaluator(nova:senior-dev) CONDITIONAL PASS** → m-1 EMPTY_DOCS Object.freeze 반영, m-3 Windows SIGINT 코멘트 반영, m-2 false-positive(watcher.ts:139-142 idempotent guard 이미 존재). typecheck PASS · 회귀 0(drift/extractor 9 fail 은 pre-existing). 변경 9 파일 + 신규 2(scripts/dev.mjs, docs/release-notes/v0.4.0-beta.2.md). | 2026-04-25T
- **세션 클로징 — v0.4.0-beta.1 공개 완료 (2026-04-24) → PASS** — 이번 세션 단일 릴리스로 **UX Audit 40 항목**(Critical 8/High 11/Medium 11/Low 2) + 사용자 피드백 4건 중 3건(#4·#6·#7) 해소. origin/main = `9aa36a1`, tag `v0.4.0-beta.1` push 완료. **GitHub Release** prerelease 공개: https://github.com/givepro91/markwand/releases/tag/v0.4.0-beta.1 (DMG arm64 `c019e5ab…4de2b` · x64 `b126823e…66b5c`). **커밋 9건**: 63ebe6e plan · aafa90d S0 · a17b8e0 S1 · 75c261b S2 · 3d874f8 S3 · 94b324f S4 · 0211f7f S5+S6+S8 · 9aa36a1 release. **하이브리드 릴리스 피벗**: OR-3 초기 승인(옵션 B 단일)에서 실행 중 S7 Annotation MVP 를 **v0.4.0-beta.2** 로 이관 — S7 이 단일 세션 서브에이전트 위임으로 완성 불가 판단 + beta.1 dogfood 피드백 을 annotation 설계에 반영 가능. **성과 수치**: appendDocs p95 1.31ms(2377 docs × 60 chunks, store Map 전환) · ProjectView chunk 1074→730KB(-32%, rehype-highlight dynamic) · 다크 탭 대비 3.41→5.05:1(WCAG AA) · 터치 타겟 24→28px. **다음 세션 재개 가이드**: (1) beta.1 dogfood 피드백 수렴 · (2) S7 Annotation MVP 착수(Plan §S7 상세 명세 있음 — dom-anchor-text-quote + CSS Custom Highlight + sidecar JSON, 노랑 1색 MVP) · (3) H11 Composer path 경고 UI wiring · (4) SshPoller dirPoll depth=2 실측(Docker sshd) · (5) 10 pre-existing test fail 정리(drift/extractor · scanner). `/nova:auto` 또는 `/nova:run` 으로 S7 이어가기 가능. | 2026-04-24T
- **v0.4.0-beta.1 Release → PASS (2026-04-24)** — **S9** release 완료: package.json 0.3.0-beta.9 → **0.4.0-beta.1**, `docs/release-notes/v0.4.0-beta.1.md` 신규(UX Audit 40 항목 해소 내역 + 성능 수치 + Known Gap), `docs/install-macos.{md,en.md}` 버전·SHA256 갱신. DMG 빌드 성공(1회, 기존 EAGAIN 미재현): **arm64** `c019e5ab89089a4755d6d13819cb143f0b7bfdb50bf7aed9144fa6cc5ef4de2b` (136MB) · **x64** `b126823e9a770c42505707af0328a2def1ee9d8b20b4d27d8276084937a66b5c` (143MB). **하이브리드 릴리스 전략 확정**: OR-3 초기 승인(옵션 B 단일)에서 **S7 Annotation MVP 를 v0.4.0-beta.2 로 이관** 피벗 — S0~S6+S8 = v0.4.0-beta.1 (UX Overhaul + SSH), S7 = v0.4.0-beta.2 (annotation 설계는 beta.1 dogfood 피드백 반영 후). 사용자 승인 완료. **총 커밋 9**: 63ebe6e plan · aafa90d S0 · a17b8e0 S1 · 75c261b S2 · 3d874f8 S3 · 94b324f S4 · 0211f7f S5+S6+S8 · (이 커밋) S9 release. 태그 push + GitHub Release prerelease 공개 예정. | 2026-04-24T
- **v0.4 S5+S6+S8 SSH Security + Wizard + Auto-refresh → PASS (2026-04-24)** — **S5**: C9 `workspace:remove` 핸들러에 `removeHostKey(id)` + `pool.dispose(id)` 배선 · H10 3-버튼 TOFU (Reject / 이번 세션만 / 영구 신뢰) + `hostKeyDb.ts` `sessionTrust` 메모리 Map(key=`${hostname}:${port}` — Critic M-5, 앱 재시작 시 휘발) · M4 SSH 모달 `aria-invalid`/`aria-describedby` 바인딩 · M7 `WorkspaceManageModal` 10초 undo 토스트 + undoBuffer · M11 Settings Experimental OFF 시 `ssh:purge-all` IPC + 체크박스 · L1 ko confirmshaming 중립화 · L2 DriftPanel 2MB skip 푸터 · `lib/tokenEstimate.ts` `estimateCost(tokens)` 신규($3/1M tokens). **S6**: H3 SshWorkspaceAddModal 2-step wizard(Step1 name+host+port+user / Step2 auth+root) + "다음/이전" + step indicator + focus trap(open 시 autofocus + Tab/Shift+Tab 순환 + trigger 복귀). **S8**: `main/transport/ssh/watcher.ts` `collectDirs(depth≤2)` + `dirPoll()` + EventEmitter `project-change` · `main/transport/ssh/index.ts` 구독 → 기존 `fs:project-change` IPC 채널 재사용(신규 채널 0). preload `respondHostKey(persistence)` + `purgeAll` 추가. i18n 16 키 ko/en 동등. typecheck PASS · vitest 262/272(신규 회귀 0). **Known Gap 이관**: H11 Composer path 경고 + 비용 옵트인 — `estimateCost` 유틸만 존재, `ComposerTray` UI wiring v0.5 이관. SshPoller `dirPoll` depth=2 실측(Docker sshd) 다음 세션 권장. | 2026-04-24T
- context compacted | 2026-04-24T13:50:55Z
- **v0.4 S4 Performance Structure → PASS (2026-04-24)** — C7 `state/store.ts` docsByProject Map + frontmatterIndex(Set<status>, Set<source>) + 모듈스코프 `cachedFlat` 변수 (Critic C-2 프로토콜 준수: Map 변경→cachedFlat 재생성→`set({docs: cachedFlat, docsByProject: new Map, frontmatterIndex: new Set})` 단일 호출). `hooks/useDocs.ts` 신규 selector 3개(`useDocsOf(pid)`, `useAllDocsFlat()`, `useFrontmatterIndex()`) + 기존 `useDocs` 리팩터. **C8 InboxView** 자체 scanDocs 루프 삭제 → useAllDocsFlat 구독 + deps `projects.map(p=>p.id).join()` hash. **H7 App.tsx** pruneStaleDocSelection/lastSelection 복원 effect 500ms setTimeout debounce. **M8 FilterBar** allStatuses/allSources for-loop → useFrontmatterIndex. **H8 MarkdownViewer** `rehype-highlight` dynamic import (useState+useEffect) + `lib/markdown.ts` top export 제거. **bench**: `scripts/bench-renderer-chunks.ts` + baseline.json 신규. 2377 docs × 60 chunks warm10 결과: **appendDocs p95 = 1.31ms** (O(N²) spread 대비 극적 개선). **번들**: ProjectView chunk 1074→**730KB (-32%)** (≤300KB 목표 미달 — mermaid/react-arborist 추가 분리는 Scope 밖). **신규 identity 테스트 7건 전원 PASS**. typecheck PASS · vitest 262 PASS/10 pre-existing fail(신규 회귀 0). **Known 잔존 위험**: Zustand 5 Map mutation 후 copy 패턴 — concurrent React 18 `startTransition` 중간 상태 이론적 노출(실측 없음, v0.5 재검토). | 2026-04-24T
- **v0.4 S3 i18n Overhaul + humanizeError 공통화 + I18nErrorBoundary → PASS (2026-04-24)** — **신규**: `src/renderer/lib/humanizeError.ts` 추출(SshWorkspaceAddModal local → 공유 모듈, 10+ SSH 에러 + FILE_TOO_LARGE/NOT_A_TEXT_DOC/drift.revalidateFailed 키 추가). `ErrorBoundary.tsx` class 확장(5 label prop: retry/restart/support/title/body + `<details>` error 접기 + 3단 CTA) + `I18nErrorBoundary` 함수형 래퍼(useTranslation 주입 — Critic M-1 패턴). `FirstRunOnboarding.tsx` 3-step 모달(📁/📦/⌨️ + ESC 닫기 + prefs.onboardingShown 영속, C3). **수정**: DriftPanel STATUS_META 를 `buildStatusMeta(t)` 로 i18n · "Finder" → t(drift.openInFinder) · 2MB skip 푸터. DriftBadge compact 비모드 "N missing/stale" → 한국어 번역(drift.badgeMissingText/Text). FilterBar STATUS_LABELS/SOURCE_LABELS 를 i18n 키 Map 으로. ImageViewer FIT_MODES {labelKey} 기반. ComposerTray/ComposerOnboarding 비개발자 카피("전체 해제"/"한 번에 보내기"). App.tsx empty.title 가치 명제 + handleAddWorkspace/handleSshSubmit 후 FirstRunOnboarding 트리거. ProjectView 3 사용처 `<ErrorBoundary>` → `<I18nErrorBoundary>`. **main/security/validators.ts**: recentDocsTab/onboardingShown/filterBarCollapsed prefs allowlist 추가. **검증**: humanizeError 사용처 4 파일(SshWorkspaceAddModal·AllProjectsView·InboxView·ProjectView)≥3 ✅ · I18nErrorBoundary mount 3 위치 ✅ · drift "missing/stale" UI 노출 0 (타입 판별만 잔존) ✅ · i18n 45 키 ko/en 동등. typecheck PASS · vitest 255/265(회귀 0). **Known Gap 이관**: drift sizeSkipped main IPC 반영(L2 ko.json 키만 준비, `main/ipc/drift.ts` 업데이트 미완 — 다음 S5 합병 예정). | 2026-04-24T
- **v0.4 S2 Viewer Layout + Keyboard a11y → PASS (2026-04-24)** — C5 사이드바 리사이즈 handle keyboard(ArrowLeft/Right ±10 · Shift+Arrow ±50 · Home=180 · End=600) + `aria-valuenow/min/max` + `tabIndex=0` + debounce 200ms (pointerDown 과 `kbResizeDebounceRef` 공유로 경쟁 차단). H5 WCAG 2.5.8 터치 타겟: Button sm 24→28px(height+minWidth+minHeight) · Toast close padding `--sp-1 --sp-2` + minW/H 28px. H6 aria-live: DriftBadge outer span `role=status aria-live=polite` · ProjectView find 카운터 동일. C4 FilterBar 기본 접기(prefs.filterBarCollapsed, 기본 true) + 단일 "필터" 토글 버튼 + 활성 칩 count 배지 · 펼침 시 기존 UI 유지. i18n `filterBar.filterToggle/filterLabel/activeCount` ko/en 동등 추가. **Evaluator (nova:qa-engineer) CONDITIONAL PASS** → Major M-2 반영(사이드바 Arrow 키 `stopPropagation` — RecentDocsPanel 탭 전환과 충돌 방지) + Minor m-3(Button sm `minHeight: 28px` 추가). Known Gap 이관: (a) find 카운터 aria-live 낭독 빈도(m-1, 120ms→300ms throttle v0.5), (b) FilterBar 전체 null 반환 시 토글 소멸(m-2 — 의도적 동작으로 기록, 필터 데이터 없으면 UI 의미 0), (c) pre-existing 10 test fail (drift/extractor·scanner — S2 외 스코프). typecheck PASS · vitest 255/265(신규 회귀 0). | 2026-04-24T
- **v0.4 S1 Tokens/CLS/RecentDocsPanel → PASS (2026-04-24)** — C6 다크 `--accent: #388bfd → #1f6feb` (white text 대비 3.41→5.05:1, WCAG AA PASS) · `--accent-hover` 를 이전 accent `#388bfd` 로 승격. `--accent-contrast: #ffffff` 토큰 신규. `globals.css` 하드코딩 `font-size:14px` / `line-height:1.5` / `::selection color:#fff` 을 토큰(`--fs-md`/`--lh-normal`/`--accent-contrast`)으로 교체. H9 이미지 CLS: SafeImage/SshImage 양쪽에 `swapAspectRatioOnLoad` 헬퍼(`naturalWidth/Height` 기반) + 초기 `aspect-ratio: 16/9` placeholder. `markdown.css` `.mermaid-block min-height: 240px` + `:has(svg)` 로 렌더 후 제거 (Chromium 130 지원). M9 RecentDocsPanel `setInterval(60s)` → **자정까지 setTimeout 재귀 스케줄** + 상대일 span `minWidth: 5ch · textAlign: right` (CLS 차단). `DriftPanel.tsx` 하드코딩 fallback `#fafaf9/#f5f5f4` 5건 토큰 교체(`var(--bg-elev)` · `var(--r-pill)` · `var(--r-lg)`). **Evaluator (nova:qa-engineer) CONDITIONAL PASS** → Major 2 반영: (1) DriftPanel:275 미정의 `--bg-elev-2` → `var(--bg-elev)`, (2) `.mermaid-block:has(svg) min-height:0` 추가. M-1(extraProps 순서)은 rehype-sanitize schema 가 img style 통과 차단이라 Plan DoD 문구 정정으로 해소. Minor 2 반영(mermaid :has(svg) · slice 생략 gap 기록). typecheck PASS · vitest 255/265(회귀 0). | 2026-04-24T
- **v0.4 S0 Test Infra → PASS (2026-04-24)** — jsdom + RTL 16 + @testing-library/jest-dom 6 + user-event 14 devDeps 추가. `vitest.config.ts` `environmentMatchGlobs` 로 `.tsx=jsdom / .ts=node` 분기. `vitest.setup.ts` 조건부 dynamic import(`typeof window !== 'undefined'`) 로 DOM matcher/cleanup 등록 + matchMedia/ResizeObserver/IntersectionObserver polyfill + react-i18next vi.mock(key identity). `src/renderer/__test-utils__/{apiMock.ts,render.tsx}` 신규 — window.api 전체 surface mock(prefs/workspace/project/fs/drift/ssh/annotation/shell/clipboard/search) + `renderWithProviders`. `src/renderer/components/ui/Button.test.tsx` 5 case 스모크. tsconfig `types: ["vite/client", "@testing-library/jest-dom"]` 추가. **Evaluator (nova:qa-engineer) CONDITIONAL PASS** → Major 2 반영: (1) vitest.setup.ts 조건부 dynamic import 로 node 환경 DOM matcher 오염 방지, (2) Plan 도 environmentMatchGlobs 방식으로 업데이트. Minor 3(apiMock node-env guard / Button toHaveStyle jsdom 제한 / render.tsx userEvent 주석) 반영. typecheck PASS · vitest 255/265(회귀 0 — baseline 250/260 대비 신규 5 추가, pre-existing 10 fail 유지). | 2026-04-24T
